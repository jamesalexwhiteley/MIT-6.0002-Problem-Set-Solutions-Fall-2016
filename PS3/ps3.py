...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        self.tiles = {} # dictionary with tuple (m,n) -> int of tile location to dirt_amount 

        for w_i in range(width):
            for h_i in range(int(height)):
                self.tiles[(w_i,h_i)] = self.dirt_amount 
    ...
    def clean_tile_at_position(self, pos, capacity):
        ...
        if capacity >= tile_dirt:
            self.tiles[(m,n)] = 0
        else:
            self.tiles[(m,n)] = tile_dirt - capacity
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        return sum(value == 0 for value in self.tiles.values())
...
# eg_position = Position(4.5,3.2)
# eg_RectRoom = RectangularRoom(10,10,50)

# eg_RectRoom.clean_tile_at_position(eg_position,10)
...
class Robot(object):
    ...
    def __init__(self, room, speed, capacity):
        ...
        # self.position = Position(random.uniform(0,room.get_width()), random.uniform(0,room.get_height()))
...
class EmptyRoom(RectangularRoom):
    ...
    def get_random_position(self):
        ...
        """
        # return random.choice(list(self.tiles))
        rand_x = random.uniform(0,self.get_width())
        rand_y = random.uniform(0,self.get_height())
...
class FurnishedRoom(RectangularRoom):
    ...
    def is_position_valid(self, pos):
        ...
        """
        m,n = math.floor(pos.get_x()), math.floor(pos.get_y())
        return (m,n) in self.tiles and (m,n) not in self.furniture_tiles
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_pos) == True:
...
class FaultyRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        new_pos = pos.get_new_position(direction, self.speed)
        if self.room.is_position_valid(new_pos) == True and self.gets_faulty() == False:
            self.set_robot_position(new_pos)
            self.room.clean_tile_at_position(new_pos, self.capacity)
        else:
            self.set_robot_direction(random.uniform(0,360))
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for num in range(num_trials):
        ...
        for ri in range(num_robots):
            # robot has random position as default
            robot = robot_type(room, speed, capacity) 
            robo_list.append(robot)
        # clean room until room cleaned by min coverage for this trial using n robots 
        while room.get_num_cleaned_tiles()/room.get_num_tiles() < min_coverage:
            time_steps += 1     
            for robot in robo_list:
                robot.update_position_and_clean()
                # anim.update(room, robo_list)
        # anim.done()
        steps_ind_trial.append(time_steps)
    return statistics.mean(steps_ind_trial)
